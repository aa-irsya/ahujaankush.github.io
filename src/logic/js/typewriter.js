...
async function typeWriter() {
  while (true) {
    if (
      typeWriterContent[tWData.currentWord].length == 1 &&
      tWData.currentWord < typeWriterContent.length
    ) {
      document
        .getElementById(tWData.writerID + tWData.currentWord)
        .appendChild(
          document.createElement(typeWriterContent[tWData.currentWord])
        );
      tWData.currentWord++;
      ...
      typeWriterContent[tWData.currentWord].length == 2 &&
      tWData.currentWord < typeWriterContent.length
    ) {
      if (
        tWData.currentChar < typeWriterContent[tWData.currentWord][0].length &&
        tWData.currentWord < typeWriterContent.length
      ) {
        document.getElementById(
          tWData.writerID + tWData.currentWord
        ).innerHTML +=
          typeWriterContent[tWData.currentWord][0][tWData.currentChar];
        tWData.currentChar++;
      } else if (
        tWData.currentChar >= typeWriterContent[tWData.currentWord][0].length &&
...
onload = async () => {
  ...
  typeWriterContent = [
    ...
    ["  // ", "color8"],
    ["DATA SCIENCE", "color6 coloredComment"],
    ["  // ", "color8"],
    ["CYBER SECURITY", "color13 coloredComment"],
  ];
